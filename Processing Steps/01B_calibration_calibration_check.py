...
# Set Folder
folder = 'data/julian/PTV_above/Calibration_Before'
# Ensure output folders exist
os.makedirs(os.path.join(folder, 'Error',
            'after_matching_plots'), exist_ok=True)
...
folder_calibration_after = os.path.join(
    os.path.dirname(folder), 'Calibration_After', 'Tests')
...
if os.path.isdir(folder_calibration_after):
    if not _has_centers(folder_calibration_after):
        print(
            f'Calibration_After: no Centers folder found under {folder_calibration_after}')
    else:
        ...
        stereomatching_after.plot_matches(os.path.join(
            folder, 'Error', 'after_matching_plots'))
else:
    print(
        f'Calibration_After folder not found: {folder_calibration_after}')  # %%
