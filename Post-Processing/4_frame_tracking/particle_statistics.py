...
plot_normalized_pdf(velocity_magnitudes,
                    variable=r'\left| \left | V \right| \right| ^2', scale=1000, output=output_path, name='vmag', fileformat='pdf', figsize=(4, 5))

# %% VELOCITY X
velocity_x = df[['vx_0', 'vx_1', 'vx_2']].mean(axis=1)
plot_normalized_pdf(velocity_x,
                    variable=r'V_\mathrm{x}', scale=1000, output=output_path, name='vx', fileformat='pdf', figsize=(4, 5))

# %% VELOCITY Y
velocity_y = df[['vy_0', 'vy_1', 'vy_2']].mean(axis=1)
plot_normalized_pdf(velocity_y,
                    variable=r'V_\mathrm{y}', scale=1000, output=output_path, name='vy', fileformat='pdf', figsize=(4, 5))

# %% VELOCITY Z
velocity_z = df[['vz_0', 'vz_1', 'vz_2']].mean(axis=1)
plot_normalized_pdf(velocity_z,
                    variable=r'V_\mathrm{z}', scale=1000, output=output_path, name='vz', fileformat='pdf', figsize=(4, 5))
...
#                     variable=r'\left| \left | a \right| \right| ^2', scale=1000, output=output_path, name='amag', log=True, fileformat='pdf',)

# %% ACCELERATION X
acceleration_x = df[['ax_0', 'ax_1']].mean(axis=1)
plot_normalized_pdf(acceleration_x,
                    variable=r'a_\mathrm{x}', unit='m/s^2', scale=1000, output=output_path, name='ax', log=True, fileformat='pdf',)

# %% ACCELERATION Y
acceleration_y = df[['ay_0', 'ay_1']].mean(axis=1)
plot_normalized_pdf(acceleration_y,
                    variable=r'a_\mathrm{y}', unit='m/s^2', scale=1000, output=output_path, name='ay', log=True, fileformat='pdf',)
# %% ACCELERATION Z
acceleration_z = df[['az_0', 'az_1']].mean(axis=1)
plot_normalized_pdf(acceleration_z,
                    variable=r'a_\mathrm{z}', unit='m/s^2', scale=1000, output=output_path, name='az', log=True, fileformat='pdf',)

# %% All Accelerations
plot_pdf([acceleration_x, acceleration_y, acceleration_z], labels=['$a_\mathrm{x}$', '$a_\mathrm{y}$', '$a_\mathrm{z}$'], scale=1000,
         variable=r'a_\mathrm{i}', xlim=[-100, 100], figsize=(6, 4), output=output_path, name='ai', log=True, unit='m/{s}^2', fileformat='pdf',)
